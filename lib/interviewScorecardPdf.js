...
export function buildInterviewScorecardPdf(examResults, meta = {}) {
  ...
  y = drawDataTable(
    ...
    [
      ...
      ...(Number(examResults.maxDescriptionScore) > 0
        ? [
            [
              "Descriptive Section",
              `${Number(examResults.descriptionScore?.score || 0).toFixed(1)} / ${examResults.maxDescriptionScore}`,
              `Grammar and error check · ${Number(examResults.descriptionScore?.errorCount) || 0} issue(s)`,
            ],
          ]
        : []),
    ],
  ...
  const drawWrappedLines = (lines, fontSize, color, lineH = 4.2) => {
    doc.setFontSize(fontSize);
    doc.setTextColor(...color);
    lines.forEach((line) => {
      ensureSpace(lineH + 1);
      doc.text(line, margin + 4, y + 3.2);
      y += lineH;
    });
  };

  if (examResults.descriptionScore?.questions?.length > 0) {
    ensureSpace(20);
    y = drawSectionTitle(doc, margin, y, "Descriptive Grammar and Error Check", COLORS.amber700);
    const descRows = examResults.descriptionScore.questions.map((qDetail) => {
      const notes = Array.isArray(qDetail.errors) && qDetail.errors.length
        ? qDetail.errors.slice(0, 2).map((e) => e.message).join("; ")
        : "No issues";
      return [
        `Question ${qDetail.questionNumber}`,
        `${Number(qDetail.score || 0).toFixed(1)} / ${qDetail.maxScore}`,
        String(qDetail.errorCount || 0),
        notes,
      ];
    });
    y = drawDataTable(
      ...
      contentW,
      [
        { header: "Question", widthPct: 0.22 },
        { header: "Score", widthPct: 0.2, align: "center" },
        { header: "Issues", widthPct: 0.14, align: "center" },
        { header: "Check", widthPct: 0.44 },
      ],
      descRows,
      { margin, headerBg: COLORS.amber700 }
    );
    ...
    ensureSpace(16);
    y = drawSectionTitle(doc, margin, y, "Descriptive Answers", COLORS.amber700);
    examResults.descriptionScore.questions.forEach((qDetail) => {
      const prompt = String(qDetail.question || "").trim() || `Question ${qDetail.questionNumber}`;
      const answer = String(qDetail.answer || "").trim() || "No answer submitted";
      const promptLines = doc.splitTextToSize(prompt, contentW - 8);
      const answerLines = doc.splitTextToSize(answer, contentW - 8);
      ensureSpace(18);
      doc.setFillColor(...COLORS.amber50);
      doc.setDrawColor(...COLORS.gray300);
      doc.setLineWidth(0.2);
      ...
      doc.setTextColor(...COLORS.gray800);
      doc.text(
        `Q${qDetail.questionNumber}  ·  ${Number(qDetail.score || 0).toFixed(1)} / ${qDetail.maxScore}`,
        margin + 3,
        y + 4.8
      );
      y += 10;
      doc.setFont("helvetica", "bold");
      drawWrappedLines(promptLines, 9, COLORS.gray900, 4.2);
      y += 1.5;
      doc.setFont("helvetica", "normal");
      drawWrappedLines(answerLines, 9, COLORS.gray800, 4.2);
      y += 5;
    });
  }
