...
import admin, { adminDb, readDocumentViaRest, shouldUseFirestoreSdkDirect } from '@/lib/firebaseAdmin';
  ...
  try {
    ...
    try {
      if (!shouldUseFirestoreSdkDirect()) {
        const userData = await readDocumentViaRest('users', decodedToken.uid);
        if (!userData) {
