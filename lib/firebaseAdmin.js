...
if (!admin.apps.length) {
  ...
  const firestoreSsl = process.env.NODE_ENV === "production";
  db.settings({
    ignoreUndefinedProperties: true,
    preferRest: true,
    ssl: firestoreSsl,
  ...
  if (firestoreSsl) {
    console.log("🔧 Firestore: production mode (SDK + SSL)");
  } else {
    console.log(
      "🔧 Firestore: local dev (SDK ssl:false; reads/writes use HTTPS REST fetch)"
    );
  }
} else {
 ...
 */
/** Production uses Firestore SDK directly; local dev uses HTTPS REST fetch (SDK ssl:false breaks on Windows). */
export function shouldUseFirestoreSdkDirect() {
...
function fromFirestoreRestValue(value) {
  ...
  if (value.mapValue !== undefined) {
    ...
    for (const key of Object.keys(fields)) {
      obj[key] = fromFirestoreRestValue(fields[key]);
    }
    return obj;
  }
...
async function readDocumentPathViaRestOnce(documentPath) {
  ...
  return firestoreRestDocToObject(doc);
}

/**
 * Read a document via HTTPS REST (used in local dev when SDK ssl:false).
 */
export async function readDocumentViaRest(collection, docId, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      return await readDocumentPathViaRestOnce(documentPath);
    } catch (e) {
      lastErr = e;
...
/**
 * Create a document in a subcollection via HTTPS REST.
 */
export async function addSubcollectionDocumentViaRest(parentPath, subcollection, data, options = {}) {
  const maxAttempts = options.maxAttempts ?? 5;
  const backoffMs = [400, 1000, 2200, 4500, 8000];
  let lastErr;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      await addSubcollectionDocumentViaRestOnce(
        parentPath,
        subcollection,
        data,
        options.documentId
...
async function addSubcollectionDocumentViaRestOnce(parentPath, subcollection, data, documentId) {
  const projectId = getFirestoreProjectId();
  const token = await getFirestoreRestAccessToken();
  if (!projectId || !token) throw new Error("Firestore REST: missing project or token");
  ...
  let url = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/${parentPath}/${subcollection}`;
  if (documentId) {
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`Firestore REST POST failed: ${res.status} ${text}`);
  }
}
...
export async function getIdentityToolkitRestAccessToken() {
  if (!_serviceAccount) return null;
  const { JWT } = await import("google-auth-library");
  const client = new JWT({
    email: _serviceAccount.client_email,
  ...
  for (const [k, v] of Object.entries(obj)) {
    ...
    if (v === null) fields[k] = { nullValue: "NULL_VALUE" };
      ...
      fields[k] = {
        arrayValue: {
          values: v.map((item) => {
            if (item === null || item === undefined) return { nullValue: "NULL_VALUE" };
...
/**
 * Create a document with auto-generated ID via HTTPS REST (local dev — avoids SDK SSL/OpenSSL).
 * @returns {Promise<string|null>} New document ID
 */
export async function addDocumentViaRest(collection, data, options = {}) {
  const maxAttempts = options.maxAttempts ?? 5;
  const backoffMs = [400, 1000, 2200, 4500, 8000];
  let lastErr;
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      return await addDocumentViaRestOnce(collection, data);
    } catch (e) {
      lastErr = e;
      if (attempt < maxAttempts && isRetryableRestNetworkError(e)) {
        await sleep(backoffMs[attempt - 1] ?? 8000);
...
async function addDocumentViaRestOnce(collection, data) {
  const projectId = getFirestoreProjectId();
  const token = await getFirestoreRestAccessToken();
  if (!projectId || !token) throw new Error("Firestore REST: missing project or token");
