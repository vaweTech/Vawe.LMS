  ...
  const fetchData = useCallback(async (user) => {
    try {
      ...
      const [studentDataResult, userRoleResult, coursesSnap] = await Promise.all([
        ...
        // Get user role and trainer chapter access
        (async () => {
          try {
            const userSnap = await getDoc(doc(db, "users", user.uid));
            if (!userSnap.exists()) {
              return { role: undefined, chapterAccess: {} };
            }
            const data = userSnap.data();
            return {
              role: data.role,
              chapterAccess: data.chapterAccess || {},
              trainerCourses: data.trainerCourses || [],
              trainerInternships: data.trainerInternships || [],
            };
          } catch (_) {
            return { role: undefined, chapterAccess: {}, trainerCourses: [], trainerInternships: [] };
          }
        })(),
      ...
      const trainerUserChapterAccess = userRoleResult.chapterAccess || {};
      ...
      if (isSuperAdmin) {
        ...
        isTrainer &&
        trainerIsAssignedToCourse(userRoleResult, { courseId })
      ) {
        merged = chapterData.map((c) => c.id);
      } else {
        const persistedTrainerChapters = isTrainer && Array.isArray(trainerUserChapterAccess[courseId])
          ? trainerUserChapterAccess[courseId]
          : [];
        merged = Array.from(
          new Set([
            ...(allowedChapters || []),
            ...(trainerAllowedChapters || []),
            ...persistedTrainerChapters,
          ])
        );
      }
