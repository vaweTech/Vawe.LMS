...
import {
  collectChapterAccessIds,
  normalizeUnlockedChapterIds,
} from "@/lib/internshipChapterAccess";
...
export default function TrainerHome() {
  ...
  const [unlockInternshipStudents, setUnlockInternshipStudents] = useState(true);
  ...
  const fetchClassStudentDocs = async (classId) => {
    if (!classId) return [];
  ...
  const getPreviouslyUnlockedChapterIds = async (
    courseId,
    { classId = null, internshipId = null, sourceCourseId = null } = {}
  ) => {
    const unlocked = new Set();

    if (classId) {
      try {
        const studentDocs = await fetchClassStudentDocs(classId);
        studentDocs.forEach((d) => {
          collectChapterAccessIds(d.data()?.chapterAccess, courseId, sourceCourseId).forEach(
            (id) => unlocked.add(id)
          );
        });
      } catch (e) {
        console.error("Error reading class student chapterAccess:", e);
      }
    }

    if (internshipId && internshipStudents.length > 0) {
      try {
        for (const row of internshipStudents) {
          const sid = row?.studentId;
          if (typeof sid !== "string" || !sid.trim()) continue;
          const sSnap = await getDoc(doc(db, "students", sid));
          if (!sSnap.exists()) continue;
          collectChapterAccessIds(sSnap.data()?.chapterAccess, courseId, sourceCourseId).forEach(
            (id) => unlocked.add(id)
          );
        }
      } catch (e) {
        console.error("Error reading internship student chapterAccess:", e);
      }
    }
    ...
    if (userId) {
      try {
        ...
        if (userSnap.exists()) {
          collectChapterAccessIds(userSnap.data()?.chapterAccess, courseId, sourceCourseId).forEach(
            (id) => unlocked.add(id)
          );
        }
      } catch (e) {
        console.error("Error reading trainer chapterAccess:", e);
      }
      ...
      try {
        ...
        console.error("Error reading today's trainer unlocks:", e);
      }
    }
  ...
  const updateTrainerChapterAccess = async (courseId, chapterIds) => {
    if (!userId || !courseId) return;
    const userRef = doc(db, "users", userId);
    await setDoc(
      userRef,
      {
        [`chapterAccess.${courseId}`]: [...new Set(chapterIds)],
      },
      { merge: true }
    );
  };

  const unionStudentChapterAccess = async (studentDocId, courseId, chapterIds) => {
    const sRef = doc(db, "students", studentDocId);
    const snap = await getDoc(sRef);
    if (!snap.exists()) {
      return { ok: false, id: studentDocId };
    }
    const unique = [...new Set((chapterIds || []).filter(Boolean))];
    for (const chId of unique) {
      await updateDoc(sRef, { [`chapterAccess.${courseId}`]: arrayUnion(chId) });
    }
    return { ok: true, id: studentDocId };
  };

  const setStudentChapterAccess = async (studentDocId, courseId, chapterIds) => {
    const sRef = doc(db, "students", studentDocId);
    const snap = await getDoc(sRef);
    if (!snap.exists()) {
      return { ok: false, id: studentDocId };
    ...
    await updateDoc(sRef, {
      [`chapterAccess.${courseId}`]: [...new Set((chapterIds || []).filter(Boolean))],
    });
    return { ok: true, id: studentDocId };
  };

  const applyUnionChapterAccessForStudents = async (studentDocIds, courseId, chapterIds) => {
    const results = await Promise.all(
      studentDocIds.map((id) => unionStudentChapterAccess(id, courseId, chapterIds))
    );
    const skipped = results.filter((r) => !r.ok).map((r) => r.id);
    return { skipped };
  };

  const applySetChapterAccessForStudents = async (studentDocIds, courseId, chapterIds) => {
    const results = await Promise.all(
      studentDocIds.map((id) => setStudentChapterAccess(id, courseId, chapterIds))
    );
    const skipped = results.filter((r) => !r.ok).map((r) => r.id);
    return { skipped };
  };
  ...
  const loadInternshipChapters = async (internshipId, courseId, preselectUnlocked = false) => {
    ...
    const courseMeta = internshipCourses.find((c) => c.id === courseId);
    const sourceCourseId = courseMeta?.sourceCourseId || "";
    try {
      ...
      if (preselectUnlocked) {
        const previouslyUnlocked = await getPreviouslyUnlockedChapterIds(courseId, {
          internshipId,
          sourceCourseId,
        });
        setSelectedInternshipChapters(
          normalizeUnlockedChapterIds(items, previouslyUnlocked)
        );
      } else {
        setSelectedInternshipChapters([]);
      }
  ...
  const loadChapters = async (courseId) => {
    ...
    try {
      ...
      setSelectedChapters(previouslyUnlocked);
    } catch (error) {
  ...
  const unlockToday = async (courseId = null) => {
    ...
    try {
      ...
      if (courseId && selectedChapters.length > 0) {
        // Unlock specific chapters — trainer first so it always succeeds
        ...
        await updateTrainerChapterAccess(courseId, selectedChapters);
        ...
        if (unlockStudents && selectedClassId && courseId) {
          const studentDocs = await fetchClassStudentDocs(selectedClassId);
          const studentIds = studentDocs.map((d) => d.id);
          const accessResult = await applyUnionChapterAccessForStudents(
            studentIds,
            courseId,
            selectedChapters
          );
          skippedStudents = accessResult.skipped;
        ...
        const skipNote =
          skippedStudents.length > 0
            ? ` (${skippedStudents.length} missing student record(s) skipped)`
            : "";
        setUnlockStatus(`Unlocked ${selectedChapters.length} chapters for today.${skipNote}`);
      } else {
        ...
        if (unlockStudents && selectedClassId && courseId) {
          try {
            ...
            await updateTrainerChapterAccess(courseId, chapterIds);

            const studentDocs = await fetchClassStudentDocs(selectedClassId);
            const studentIds = studentDocs.map((d) => d.id);
            await applyUnionChapterAccessForStudents(studentIds, courseId, chapterIds);
  ...
  const handleInternshipChapterUnlock = async () => {
    ...
    try {
      ...
      if (selectedInternshipChapters.length > 0) {
        ...
        await updateTrainerChapterAccess(
          selectedInternshipCourseForUnlock.id,
        ...
        if (studentIds.length > 0) {
          const accessResult = await applySetChapterAccessForStudents(
            studentIds,
            selectedInternshipCourseForUnlock.id,
            selectedInternshipChapters
          );
        ...
        const skipNote =
          skippedStudents.length > 0
            ...
            : "";
        setUnlockStatus(
          `Unlocked ${selectedInternshipChapters.length} chapters for internship${unlockInternshipStudents ? " and students" : ""} on ${ymd}.${skipNote}`
        );
      } else {
        ...
        if (studentIds.length > 0) {
          await applySetChapterAccessForStudents(
            studentIds,
            selectedInternshipCourseForUnlock.id,
            []
          );
        ...
        await updateTrainerChapterAccess(selectedInternshipCourseForUnlock.id, []);
