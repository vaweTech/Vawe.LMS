...
import {
  linkSessionPhoneKey,
  readStoredLinkAssignment,
  resolveInterviewLinkAssignment,
} from "@/lib/interviewLinkAssignment";
...
export default function TakeInterviewExamPage() {
  ...
  const [examLinkLabel, setExamLinkLabel] = useState("");
  const [assignedExamTitle, setAssignedExamTitle] = useState("");
  ...
  const applyLinkAssignment = useCallback(
    async (phoneDigits) => {
      const linkToken = searchParams?.get("linkToken") || "";
      const normalized = normalizeInterviewPhone(phoneDigits);
      if (!linkToken || normalized.length < 10) return null;

      const assignment = await resolveInterviewLinkAssignment({
        linkToken,
        phone: normalized,
      });
      if (!assignment?.examId) return null;

      if (assignment.linkLabel) {
        setExamLinkLabel(assignment.linkLabel);
        sessionStorage.setItem(
          `interview-link-label:${linkToken}`,
          assignment.linkLabel
        );
      }
      setAssignedExamTitle(assignment.examTitle || "");

      sessionStorage.setItem(linkSessionPhoneKey(linkToken), normalized);

      if (String(assignment.examId) !== String(examId)) {
        router.replace(
          `/interview/${assignment.examId}?linkToken=${encodeURIComponent(linkToken)}`
        );
      }
      return assignment;
    },
    [searchParams, examId, router]
  );

  useEffect(() => {
    async function resolveLinkMeta() {
      const linkToken = searchParams?.get("linkToken") || "";
      if (!linkToken) {
        setExamLinkLabel("");
        setAssignedExamTitle("");
        return;
      }

      const sessionPhone =
        typeof window !== "undefined"
          ? sessionStorage.getItem(linkSessionPhoneKey(linkToken))
          : "";
      if (sessionPhone) {
        setPhone(sessionPhone);
        setPhoneVerified(true);
        const stored = readStoredLinkAssignment(linkToken, sessionPhone);
        if (stored?.linkLabel) setExamLinkLabel(stored.linkLabel);
        if (stored?.examTitle) setAssignedExamTitle(stored.examTitle);
        if (stored?.examId && String(stored.examId) !== String(examId)) {
          router.replace(
            `/interview/${stored.examId}?linkToken=${encodeURIComponent(linkToken)}`
          );
          return;
        }
      }
      ...
      if (typeof window !== "undefined") {
        const cachedLabel = sessionStorage.getItem(
          `interview-link-label:${linkToken}`
        );
        if (cachedLabel) setExamLinkLabel(cachedLabel);
      }
      ...
      try {
        ...
        const label = String(snap.data()?.label || "").trim();
        if (label) {
          setExamLinkLabel(label);
          if (typeof window !== "undefined") {
            sessionStorage.setItem(`interview-link-label:${linkToken}`, label);
          }
        }
      } catch {
        setExamLinkLabel("");
  ...
  const examDisplayTitle = examLinkLabel || exam?.title || "Interview Exam";
  const examAssignedLine =
    examLinkLabel && (assignedExamTitle || exam?.title)
      ? `Assigned set: ${assignedExamTitle || exam?.title}`
      : "";
  ...
  const downloadScorecard = async () => {
    ...
    const scorecardTitle = examDisplayTitle;
    const doc = buildInterviewScorecardPdf(examResults, {
      examTitle: scorecardTitle,
      assignedSetLine: examAssignedLine || undefined,
      fullName,
      phone,
      logoBase64,
    });

    doc.save(buildScorecardFileName(fullName));
  };
  ...
  useEffect(() => {
    ...
    // iPhone/iPad do not support the Fullscreen API for normal web pages.
  ...
  const handleVerifyWhatsappOtp = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPhoneOtpValue("");
        setOtpSendError("");
        const verifiedPhone = normalizeInterviewPhone(otpPhoneSent || phone);
        await applyLinkAssignment(verifiedPhone);
        return;
      }
  ...
  const startExam = async () => {
    ...
    // iPhone/iPad: skip fullscreen (API unsupported) — start with timer + visibility monitoring only.
    ...
    if (typeof document !== "undefined" && !document.fullscreenElement) {
      ...
      if (ok || document.fullscreenElement) {
        // Fullscreen entered — start immediately (do not rely only on fullscreenchange timing).
        setPendingStart(false);
        setStarted(true);
        startTimer(attempt.remainingMs);
        return;
      }
      // Fullscreen denied/unavailable — do not leave the exam stuck in pendingStart.
      setPendingStart(false);
      setStarted(true);
      startTimer(attempt.remainingMs);
      return;
  ...
  if (!exam) {
    return <div className="min-h-dvh flex items-center justify-center">Exam not found.</div>;
  }

  return (
    <div className="min-h-dvh bg-gradient-to-b from-sky-50 to-cyan-50">
      ...
      {showResults && examResults && (
        ...
        >
          ...
          <div className="min-h-dvh flex items-center justify-center p-4 py-6">
            <div className="w-full max-w-2xl">
              {/* Header */}
              <div className="text-center mb-4">
                <h1 className="text-2xl sm:text-3xl font-bold text-[#0c4a6e] mb-1">{examDisplayTitle}</h1>
                {examAssignedLine ? (
                  <p className="text-sm sm:text-base text-gray-600">{examAssignedLine}</p>
                ) : (
      ...
      {!showResults && !processing && (
        <>
          {/* Fullscreen Required Overlay — hidden on iPhone/iPad (Fullscreen API unsupported) */}
          {started &&
            !isAppleMobileDevice() &&
          ...
          {/* Tab Switch Warning Overlay — clearer 1st / 2nd strike messaging */}
              ...
              <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 text-center">
                <div
                  className={`inline-flex items-center justify-center w-20 h-20 rounded-full mb-4 ${
                    tabSwitchCount >= 2 ? "bg-red-100" : "bg-amber-100"
                  }`}
                >
                  <svg
                    className={`w-10 h-10 ${tabSwitchCount >= 2 ? "text-red-600" : "text-amber-600"}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                <p
                  className={`text-xs font-bold uppercase tracking-wide mb-2 ${
                    tabSwitchCount >= 2 ? "text-red-600" : "text-amber-700"
                  }`}
                >
                ...
                <h2 className="text-2xl font-bold text-gray-900 mb-2">
                  {tabSwitchCount <= 1
                    ? "First warning — stay on this tab"
                    : tabSwitchCount === 2
                      ? "Final warning — one chance left"
                      : "Exam will be blocked"}
                </h2>

                <p className="text-sm text-gray-600 mb-4">
                  Leaving this tab or app was detected. You have used{" "}
                  <span className="font-semibold text-gray-900">{Math.min(tabSwitchCount, 3)}</span> of{" "}
                  <span className="font-semibold text-gray-900">3</span> allowed switches.
                </p>
                ...
                {tabSwitchCount < 3 ? (
                  ...
                  >
                    {tabSwitchCount <= 1
                      ? "You have 2 chances left. A 3rd leave will permanently block this exam."
                      : "You have 1 chance left. One more leave will permanently block this exam."}
                  </div>
                ) : (
                  <div className="rounded-lg px-4 py-3 mb-4 text-sm font-semibold bg-red-50 text-red-800 border border-red-200">
                    Limit reached. Contact an administrator to unblock.
                  </div>
                )}

                <p className="text-xs text-gray-500 mb-6">
                  {isAppleMobileDevice()
                    ? "Return to this tab and tap Continue to resume."
                ...
                <button
                  ...
                  className={`w-full sm:w-auto min-w-[180px] px-6 py-2.5 text-white rounded-lg font-medium transition-colors ${
                    tabSwitchCount >= 2
              ...
              <div className="flex items-center gap-3">
                {started && tabSwitchCount > 0 && (
                  <span
                    ...
                    title="Tab / app leave warnings. 3rd leave blocks the exam."
                  >
                    {tabSwitchCount <= 1
                      ? "Warning 1/3 · 2 left"
                      : tabSwitchCount === 2
                        ? "Final warning 2/3 · 1 left"
                        : "Blocked 3/3"}
                  </span>
                )}
          ...
          <div className="bg-white rounded-xl shadow p-4 sm:p-6 mb-6">
            ...
            <div className="mt-6">
              ...
              <ul className="list-disc pl-5 text-sm text-gray-700 space-y-1">
                ...
                <li><strong>Important:</strong> Leaving this tab/app counts as a warning. Warning 1 and 2 show a clear alert. The <strong>3rd</strong> leave permanently blocks the exam.</li>
              </ul>
