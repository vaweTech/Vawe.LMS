...
function formatAccountCreated(value) {
...
export default function StudentListPage() {
  ...
  // Students filtered by search, lock, role, department, course and batch (for table and summary)
  const filteredStudents = students.filter((s) => {
    if (searchQuery.trim()) {
      ...
      const phone = String(s.phone1 || s.phone || s.phone2 || "").replace(/\D/g, "");
      const matchName = name.includes(q);
      const matchEmail = email.includes(q);
      const matchPhone = qDigits.length > 0 && phone.includes(qDigits);
    ...
    if (roleFilter && !matchesStudentRoleFilter(s, roleFilter)) return false;
  ...
  async function handleDeleteStudent(id) {
    ...
    try {
      const result = await executeStudentDelete({
        ...
        student,
        studentId: id,
        collegeSubdomain: student?.collegeSubdomain || null,
        requestCollegeSubdomain: null,
  ...
  function razorpayCheckoutKey(orderData) {
    return orderData?.key_id || process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID || "";
  }

  function showRazorpayFailure(error) {
    const desc = String(error?.description || error?.reason || error?.message || "").trim();
    if (
      desc.toLowerCase().includes("not allowed to accept payments") ||
      desc.toLowerCase().includes("website")
    ) {
      alert(
        "Razorpay blocked this payment on www.skillwins.in.\n\n" +
          "Fix in Razorpay Dashboard (Live mode):\n" +
          "1. Settings → Website & App Settings\n" +
          "2. Add https://www.skillwins.in and https://skillwins.in\n" +
          "3. Ensure Vercel env vars RAZORPAY_KEY_ID, RAZORPAY_KEY_SECRET, and NEXT_PUBLIC_RAZORPAY_KEY_ID use the same Live key pair\n" +
          "4. Redeploy after updating env vars"
      );
      return;
    }
    alert(desc || "Online payment failed. Try again or use Cash payment.");
  }
  ...
  async function handlePayFee() {
    ...
    try {
      ...
      const checkoutKey = razorpayCheckoutKey(orderData);
      if (!checkoutKey) {
        throw new Error(
          "Razorpay key is not configured. Set NEXT_PUBLIC_RAZORPAY_KEY_ID on Vercel and redeploy."
        );
      }
      ...
      const options = {
        ...
        description: Array.isArray(selectedStudent.coursesTitle)
          ? selectedStudent.coursesTitle.join(", ")
          : selectedStudent.coursesTitle || "Payment",
        ...
        handler: async function (response) {
          try {
            const verifyResponse = await makeAuthenticatedRequest(
              "/api/payments/razorpay/verify-admin",
              {
                method: "POST",
                body: JSON.stringify({
            ...
            const params = new URLSearchParams({
              ...
              course: Array.isArray(selectedStudent.coursesTitle)
                ...
                : selectedStudent.coursesTitle || "",
              totalFee: String(Math.round(totalFee * 100)),
              paidFee: String(Math.round((verifyData.newPaid ?? paidFee + amount) * 100)),
              dueAmount: String(Math.round((verifyData.remainingDue ?? Math.max(totalFee - paidFee - amount, 0)) * 100)),
              date: new Date().toISOString(),
      ...
      rzp.on("payment.failed", function (resp) {
        ...
        showRazorpayFailure(resp?.error);
        setIsProcessingPayment(false);
      });
      rzp.open();
    } catch (err) {
    ...
    <CheckAdminAuth>
      <div className="mx-auto p-6 bg-white shadow-md rounded-md">
        ...
        <div className="mb-4 flex items-center gap-2 flex-wrap">
          ...
          {(roleFilter || searchQuery || lockFilter !== "all" || deptFilter || courseFilter || batchFilter) && (
        ...
        {students.length === 0 ? (
          ...
          <p className="text-gray-500 text-center">No students match the current filters.</p>
        ) : (
            ...
            <table className="w-full border-collapse border text-sm">
              <thead>
                <tr className="bg-gray-200">
                  ...
                  <th className="border p-2 whitespace-nowrap">Account Created</th>
                ...
                {filteredStudents.map((s, index) => (
                  <tr key={s.id} className="hover:bg-gray-50">
                    ...
                    <td className="border p-2 whitespace-nowrap">
                      {formatAccountCreated(s.createdAt)}
                    </td>
                    <td className="border p-2">
                  ...
                  <td className="border p-2 text-center">
                    ...
                    <button
                      onClick={() => {
                        const isInternship = isInternshipStudentDoc(s);
                        const courseTitle = Array.isArray(s.coursesTitle)
                          ? s.coursesTitle.join(", ")
                          : s.coursesTitle || "Course";
                        const params = new URLSearchParams({
                          name: s.name || "",
                          course: courseTitle,
                        ...
                        if (isInternship) {
                          ...
                          const regNo =
                            s.regdNo || s.regNo || s.registrationNumber || s.regnNo || "";
                          if (regNo) params.set("regNo", regNo);
                          const college = s.college || s.collegeUniversity || "";
                          if (college) params.set("college", college);
                          if (s.dateOfJoining) params.set("from", s.dateOfJoining);
                          if (s.dateOfCompletion || s.internshipTo) {
                            params.set("to", s.dateOfCompletion || s.internshipTo);
                          }
                        }
        ...
        {students.length > 0 && (
          <div className="mt-6 bg-gray-50 p-4 rounded-lg border">
            <h3 className="text-lg font-semibold mb-4 text-center text-gray-800">
              📊 Fee Summary {roleFilter || searchQuery || lockFilter !== "all" ? `(filtered: ${filteredStudents.length})` : ""}
            </h3>
