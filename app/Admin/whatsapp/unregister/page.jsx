  ...
  async function handleSend() {
    ...
    const recipients = eligible
      .map((row) => ({
        name: row.name,
        phone: row.mobile,
        bodyParams: [sanitizeParam(row.name, 100), bodyText],
      }))
      .filter((r) => r.bodyParams[0] && r.phone);
    ...
    try {
      ...
      errors.push({ name: "Bulk", mobile: "", error: e?.message || "Request failed" });
    }
        ...
        {parsed.length > 0 && (
          <>
              <p className="text-xs text-gray-500 mt-1">
                custom_message uses language <strong>en</strong> (set automatically on the server).
              </p>
            <div className="mb-6">
