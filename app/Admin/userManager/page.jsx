...
import { executeStudentDelete } from "@/lib/deleteStudentClient";
import {
  isCrtStudentRole,
  ...
  getStudentRoles,
  studentHasCrtRole,
  studentHasInternshipRole,
  studentHasSkillwinsRole,
  resolveStudentRoleFieldsForSave,
...
function matchesStudentSearch(student, rawQuery) {
  ...
  const phone = String(student?.phone1 || student?.phone || student?.phone2 || "").replace(
    /\D/g,
    ""
  );
  return (
...
function renderStudentRoleCell(student) {
  const hasInternship = studentHasInternshipRole(student);
  const hasCrt = studentHasCrtRole(student);
  const hasSkillwins = studentHasSkillwinsRole(student);
  const hasLmsStudent =
    !hasInternship && !hasCrt && !hasSkillwins && isScopedStudentRole(inferStudentRole(student));
...
export default function UserManagerPage() {
  ...
  const fetchClasses = useCallback(async () => {
    const snap = await getDocs(collection(db, ...tenantSegments(null, "classes")));
  ...
  const fetchCourses = useCallback(async () => {
    const snap = await getDocs(collection(db, ...tenantSegments(null, "courses")));
  ...
  async function handleAddClass(e) {
    ...
    await addDoc(collection(db, ...tenantSegments(null, "classes")), newClass);
    setNewClass({ name: "" });
    fetchClasses();
  }
  ...
  /** LMS pool: vaweStudent / vaweInternship (and dual CRT+internship). */
  ...
  const studentsForTable = useMemo(() => {
    if (roleFilter === "crtStudent") {
      return students.filter((s) => studentHasCrtRole(s));
    }
    if (roleFilter === "student") {
      return lmsStudents.filter((s) => isRegularStudentDoc(s));
    }
    if (roleFilter === "internship") {
      return students.filter((s) => isInternshipStudentDoc(s));
    }
    return lmsStudents;
  }, [students, lmsStudents, roleFilter]);

  const filteredStudents = useMemo(() => {
    return assignableStudents.filter((s) => matchesStudentSearch(s, searchEmail));
  }, [assignableStudents, searchEmail]);

  const searchedStudentsForTable = useMemo(() => {
    return studentsForTable.filter((s) => matchesStudentSearch(s, studentTableSearch));
  }, [studentsForTable, studentTableSearch]);

  const sortedStudents = useMemo(() => [...searchedStudentsForTable].sort((a, b) => {
    if (sortBy === "regNo") {
  ...
  }), [searchedStudentsForTable, sortBy]);

  const roleCounts = useMemo(
    () => ({
      all: lmsStudents.length,
      student: lmsStudents.filter((s) => isRegularStudentDoc(s)).length,
      internship: students.filter((s) => isInternshipStudentDoc(s)).length,
      crt: students.filter((s) => studentHasCrtRole(s)).length,
    }),
  ...
  function openEditStudent(student) {
    ...
    const wasCrt = studentHasCrtRole(student);
    const registrationNumber =
      student.registrationNumber || student.regdNo || student.regNo || student.regnNo || "";
    setEditStudent({
      ...student,
      registrationNumber,
      roles,
      isInternship: roles.some((r) => isScopedInternshipRole(r)),
      isCrt: roles.some((r) => isCrtStudentRole(r)),
      _wasCrt: wasCrt,
    });
    setShowEditStudentModal(true);
  }

  function updateEditStudentRoles(nextFlags) {
    const roles = buildStudentRolesFromFlags({
      isCrt: !!nextFlags.isCrt,
      isInternship: !!nextFlags.isInternship,
      isSkillwins: !!nextFlags.isSkillwins,
      collegeSubdomain: tenantSubdomain,
    });
    setEditStudent((prev) => ({ ...prev, ...nextFlags, roles }));
  }
  ...
  async function handleSaveStudentEdit(e) {
    ...
    const registrationNumber = String(editStudent.registrationNumber ?? "").trim();
    if (!registrationNumber) {
      alert("Registration number is required and cannot be empty.");
      return;
    }

    try {
      const studentRef = doc(db, "students", editStudent.id);
      const studentTenant = editStudent.collegeSubdomain || null;
      const roleFields = resolveStudentRoleFieldsForSave({
        isCrt: !!editStudent.isCrt,
        isInternship: !!editStudent.isInternship,
        isSkillwins: !!editStudent.isSkillwins,
        collegeSubdomain: studentTenant,
      });
      ...
      const updateData = {
        ...
        registrationNumber,
        regdNo: registrationNumber,
        roles: roleFields.roles,
        role: roleFields.role,
        isCrt: roleFields.isCrt,
        isInternship: roleFields.isInternship,
        isSkillwins: roleFields.isSkillwins,
        portal: roleFields.portal,
      };

      if (roleFields.isCrt) {
        updateData.collegeSubdomain = collegeSubdomain || tenantSubdomain;
      }
      ...
      const studentUid = editStudent.uid || editStudent.id;
      const rosterRef = doc(db, ...crtStudentRosterDocSegments(collegeSubdomain, studentUid));
      ...
      if (roleFields.isCrt) {
        await setDoc(
          ...
          {
            uid: studentUid,
            studentDocId: editStudent.id,
            regNo: updateData.registrationNumber || "",
            regdNo: updateData.registrationNumber || "",
            ...
            isCrt: true,
            isInternship: roleFields.isInternship,
            phone1: updateData.phone || "",
            phone: updateData.phone || "",
            password: editStudent.password || "",
            collegeSubdomain: collegeSubdomain || tenantSubdomain,
            updatedAt: new Date().toISOString(),
          },
          { merge: true }
        );
      } else if (editStudent._wasCrt) {
  ...
  async function handleDeleteStudent(student) {
    ...
    const confirmed = confirm(
      `Delete ${student.name || "this student"} from the system? This will also remove their login access and CRT roster entry.`
    );
    if (!confirmed) return;

    try {
      const result = await executeStudentDelete({
        firestore: { db, doc, getDoc, getDocs, collection, deleteDoc, writeBatch, query, where },
        student,
      ...
      <div className="p-8 bg-gray-100 min-h-screen">
        ...
        <div className="bg-white p-6 rounded shadow">
          <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <input
                ...
                className="border p-2 rounded text-sm w-full sm:w-72"
              />
              ...
              >
                ...
                <option value="student">
                  Student — {getScopedStudentRole(null)} ({roleCounts.student})
                </option>
                <option value="internship">
                  Internship — {getScopedInternshipRole(null)} ({roleCounts.internship})
                </option>
              ...
              <span className="text-sm text-gray-500">
                {roleFilter || studentTableSearch.trim()
          ...
          {students.length === 0 ? (
            ...
            <p className="text-gray-500">No students match the current filters or search.</p>
          ) : (
            <div className="overflow-x-auto">
                ...
                <tbody>
                  {sortedStudents.map((s) => (
                    <tr key={s.id}>
                      ...
                      <td className="border p-2">{renderStudentRoleCell(s)}</td>
              ...
              <form onSubmit={handleSaveStudentEdit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditStudent({ ...editStudent, registrationNumber: e.target.value })
                    }
                    className="border p-2 rounded w-full"
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-4">
                    <label className="flex items-center gap-2 text-sm text-gray-700">
                      <input
                        type="checkbox"
                        checked={!!editStudent.isInternship}
                        onChange={(e) =>
                          updateEditStudentRoles({
                            isCrt: !!editStudent.isCrt,
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Leave both unchecked for a regular student. You can select Internship, CRT, or
                    both.
                  </p>
                  <p className="text-xs text-gray-400 mt-1">
                    Current: {formatStudentRoleLabelsForDoc(editStudent)}
                  </p>
                  {Array.isArray(editStudent.roles) && editStudent.roles.length > 0 && (
                    <p className="text-xs text-gray-400 mt-1 break-all">
                      Stored roles: {editStudent.roles.join(", ")}
                    </p>
                  )}
                </div>
