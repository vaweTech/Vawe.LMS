...
import {
  executeDeleteAllInterviewResults,
  executeDeleteInterviewResult,
} from "@/lib/deleteInterviewResultsClient";
...
export default function AdminInterviewExamsPage() {
  ...
  const [linkExamIds, setLinkExamIds] = useState([]);
  ...
  const fetchExamLinks = useCallback(async () => {
    ...
    try {
      const snap = await firestoreHelpers.getDocs(
        firestoreHelpers.collection(db, "interviewExamLinks")
      );
      const rows = snap.docs.map((d) => {
        ...
        return {
          id: d.id,
          label: data?.label || "",
          paperExamIds: getLinkPaperExamIds(data),
  ...
  const createExamLink = async () => {
    if (linkExamIds.length < 1) {
      alert("Select at least one exam for this link.");
      return;
    }
    setCreatingLink(true);
    try {
      const linkId = crypto.randomUUID().replace(/-/g, "").slice(0, 16);
      const paperExamTitles = linkExamIds.map(
        (id) => exams.find((e) => e.id === id)?.title || "Untitled exam"
      );
      const payload = {
        label: String(linkLabel || "").trim(),
        paperExamIds: [...linkExamIds],
        paperExamTitles,
        active: true,
        createdAt: Date.now(),
      ...
      await firestoreHelpers.setDoc(
        firestoreHelpers.doc(db, "interviewExamLinks", linkId),
        payload
      );
      setLinkLabel("");
      setLinkExamIds([]);
      await fetchExamLinks();
      alert("Exam link created. Share the URL or QR code — candidates get a random test from your selection.");
    } catch (e) {
      console.error(e);
      alert("Failed to create exam link. Please try again.");
    } finally {
      setCreatingLink(false);
    }
  };

  const copyJoinLink = async (linkId) => {
    const url = buildInterviewJoinUrl(linkId, linkOrigin);
    try {
  ...
  const toggleLinkActive = async (link) => {
    try {
      ...
      await firestoreHelpers.updateDoc(
        firestoreHelpers.doc(db, "interviewExamLinks", link.id),
        { active: next }
      );
      setExamLinks((prev) =>
        prev.map((l) => (l.id === link.id ? { ...l, active: next } : l))
      );
    } catch (e) {
  ...
  const deleteExamLink = async (link) => {
    const label = link.label || link.id;
    if (!confirm(`Delete exam link "${label}"? Candidates will no longer be able to use it.`)) return;
    try {
      await firestoreHelpers.deleteDoc(
        firestoreHelpers.doc(db, "interviewExamLinks", link.id)
      );
      setExamLinks((prev) => prev.filter((l) => l.id !== link.id));
    } catch (e) {
  ...
  const interviewFirestore = useMemo(
    () => ({
      db,
      doc: firestoreHelpers.doc,
      collection: firestoreHelpers.collection,
      getDocs: firestoreHelpers.getDocs,
      deleteDoc: firestoreHelpers.deleteDoc,
    }),
    []
  );
  ...
  const deleteResult = async (examId, submissionId, name, phone) => {
    ...
    try {
      const result = await executeDeleteInterviewResult({
        firestore: interviewFirestore,
        examId,
        submissionId,
      ...
      alert(e?.message || "Failed to delete result. Please try again.");
    }
  };
  ...
  const deleteAllResults = async () => {
    ...
    try {
      const result = await executeDeleteAllInterviewResults({
        firestore: interviewFirestore,
        examIds: examsToDelete.map((ex) => ex.id),
        makeAuthenticatedRequest,
        onRefresh: fetchResults,
      });
      if (result.cancelled) return;
      alert(`Successfully deleted ${result.deletedCount ?? totalCount} result(s).`);
    } catch (e) {
      alert(e?.message || "Failed to delete results. Please try again.");
  ...
  useEffect(() => {
    if (activePanel === "links") fetchExamLinks();
  }, [activePanel, fetchExamLinks]);
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            {[
              ...
              { id: "links", label: "Exam links", icon: Link2 },
...
- # → compiler receives: (space)`}
          ...
          <div className="space-y-6 mb-8">
            <div className="bg-white rounded-2xl border border-gray-200/90 shadow-sm p-4 sm:p-6">
              <div className="flex flex-wrap items-center justify-between gap-3 mb-6 pb-4 border-b border-gray-100">
                <div>
                  <h2 className="text-lg font-semibold text-gray-900">Random exam link</h2>
                  <p className="text-xs text-gray-500 mt-0.5 max-w-2xl">
                    Select multiple exams below. Each candidate who opens the link or scans the QR code is assigned one exam at random from your selection.
                  </p>
                </div>
              </div>
              ...
              <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Campus drive – Batch A"
                    className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-white focus:ring-2 focus:ring-[#00448a]/20 focus:border-[#00448a] outline-none mb-4"
                  />

                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
                    Select exams ({linkExamIds.length} selected)
                  </label>
                  {exams.length === 0 ? (
                    <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50/80 py-8 px-4 text-center">
                      <p className="text-sm text-gray-600">Create exams in <strong>Compose</strong> first.</p>
                    </div>
                  ) : (
                    <ul className="max-h-72 overflow-y-auto space-y-2 rounded-xl border border-gray-100 bg-gray-50/40 p-3">
                      {exams.map((ex) => {
                        const checked = linkExamIds.includes(ex.id);
                        return (
                          <li key={ex.id}>
                            <label
                              className={`flex items-start gap-3 rounded-xl border px-3 py-2.5 cursor-pointer transition-colors ${
                                checked
                                  ? "border-[#00448a]/40 bg-[#00448a]/5"
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={checked}
                                onChange={() => toggleLinkExam(ex.id)}
                                className="mt-1 rounded border-gray-300 text-[#00448a] focus:ring-[#00448a]"
                              />
                              <span className="min-w-0">
                                <span className="block text-sm font-semibold text-gray-900 truncate">{ex.title}</span>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button
                      type="button"
                      onClick={() => setLinkExamIds(exams.map((e) => e.id))}
                      disabled={exams.length === 0}
                      className="px-3 py-2 text-sm font-medium rounded-xl border border-gray-200 bg-white text-gray-800 hover:border-[#00448a]/40 disabled:opacity-50"
                    >
                      Select all
                    </button>
                    <button
                      type="button"
                      onClick={() => setLinkExamIds([])}
                      className="px-3 py-2 text-sm font-medium rounded-xl border border-gray-200 bg-white text-gray-800 hover:border-[#00448a]/40"
                    >
                      Clear
                    </button>
                  </div>
                  ...
                  <button
                    type="button"
                    onClick={createExamLink}
                    disabled={creatingLink || linkExamIds.length === 0}
                    className="mt-5 w-full sm:w-auto px-6 py-3 text-sm font-semibold bg-[#00448a] hover:bg-[#003a76] text-white rounded-xl disabled:opacity-60 shadow-sm transition-colors"
                  >
                    {creatingLink ? "Generating…" : "Generate link & QR code"}
                  </button>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200/90 shadow-sm p-4 sm:p-6">
              <div className="flex flex-wrap items-center justify-between gap-3 mb-6 pb-4 border-b border-gray-100">
                <div>
                  <h2 className="text-lg font-semibold text-gray-900">Your exam links</h2>
                  <p className="text-xs text-gray-500 mt-0.5">
                    {linksLoading ? "Loading…" : `${examLinks.length} link(s)`}
                  </p>
                </div>
                <button
                  type="button"
                  onClick={fetchExamLinks}
              ...
              {examLinks.length === 0 ? (
                <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50/80 py-12 px-4 text-center">
                  <p className="text-sm text-gray-600">No exam links yet. Create one above to get a shareable URL and QR code.</p>
                </div>
              ) : (
                <ul className="space-y-4">
                  {examLinks.map((link) => {
                    const joinUrl = buildInterviewJoinUrl(link.id, linkOrigin);
                    const qrUrl = qrCodeImageUrl(joinUrl, 200);
                    const examTitles = link.paperExamIds.map(
                      (id) => exams.find((e) => e.id === id)?.title || id
                    );
                    const created =
                    ...
                    return (
                      <li
                        key={link.id}
                        className="rounded-xl border border-gray-100 bg-gray-50/40 p-4 sm:p-5"
                      >
                        <div className="flex flex-col lg:flex-row lg:items-start gap-5">
                          <div className="shrink-0 flex flex-col items-center gap-2">
                            <Image
                              src={qrUrl}
                              alt={`QR code for ${link.label || "exam link"}`}
                              width={200}
                              height={200}
                              unoptimized
                            ...
                            <a
                              href={qrUrl}
                              download={`exam-link-${link.id}.png`}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="text-xs font-medium text-[#00448a] hover:underline"
                            >
                              Download QR
                            </a>
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500 mb-2">Created {created}</p>
                            <p className="text-xs text-gray-600 mb-3">
                              <span className="font-medium text-gray-700">Exams in pool:</span>{" "}
                              {examTitles.join(", ") || "—"}
                            </p>
                            ...
                            <div className="flex flex-wrap items-center gap-2 mb-3">
                              <input
                                ...
                                readOnly
                                value={joinUrl}
                                className="flex-1 min-w-[200px] border border-gray-200 rounded-lg px-3 py-2 text-xs bg-white font-mono text-gray-700"
                              />
                              <button
                                type="button"
                                onClick={() => copyJoinLink(link.id)}
                                className="px-3 py-2 text-sm font-semibold rounded-lg border border-gray-200 bg-white text-gray-800 hover:border-[#00448a]/40 hover:text-[#00448a] transition-colors shrink-0"
                              >
                              ...
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="px-3 py-2 text-sm font-semibold rounded-lg bg-[#00448a] text-white hover:bg-[#003a76] transition-colors shrink-0"
                              >
                                Open
                              </a>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              <button
                                ...
                                className="px-3 py-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 hover:border-[#00448a]/40 transition-colors"
                              >
                                {link.active ? "Deactivate" : "Activate"}
                              </button>
                              <button
                                type="button"
                                onClick={() => deleteExamLink(link)}
                                className="px-3 py-2 text-sm font-semibold rounded-lg border border-red-200 text-red-700 bg-white hover:bg-red-50 transition-colors"
                              >
                                Delete
                              </button>
