...
import {
  ...
  getCrtStudentRoleQueryValues,
  mergeCrtStudentsForAdmin,
...
export default function CRTStudentUserManagementPage() {
  ...
  const fetchStudents = useCallback(async () => {
    ...
    try {
      const crtRoleValues = getCrtStudentRoleQueryValues(collegeSubdomain);
      const collectionPaths = getStudentCollectionSegmentVariants(
        collegeSubdomain,
        collegeSubdomain
      );

      const rosterSnap = await firestoreHelpers.getDocs(
        firestoreHelpers.collection(
          db,
          ...crtStudentRosterCollectionSegments(collegeSubdomain)
        )
      );

      const byDocKey = new Map();
      await Promise.all(
        collectionPaths.map(async (segs) => {
          const studentsRef = firestoreHelpers.collection(db, ...segs);
          const [flagSnap, roleSnap] = await Promise.all([
            ...
            firestoreHelpers.getDocs(
              firestoreHelpers.query(
                studentsRef,
                firestoreHelpers.where("role", "in", crtRoleValues)
              )
            ),
          ]);
          [...flagSnap.docs, ...roleSnap.docs].forEach((d) => {
            const key = `${segs.join("/")}/${d.id}`;
            byDocKey.set(key, { id: d.id, ...d.data() });
          });
        })
      );

      const rosterList = rosterSnap.docs.map((d) => ({ id: d.id, ...d.data() }));
      const collectionList = normalizeStudentsForAdmin([...byDocKey.values()]);
      setStudents(mergeCrtStudentsForAdmin(rosterList, collectionList));
    } catch (err) {
      console.error(err);
  ...
  const handleAdmissionSubmit = async (e) => {
    ...
    const dobCheck = validateDateOfBirth(toDateInputString(dateOfBirth));
    if (!dobCheck.valid) {
      alert(dobCheck.message);
      return;
    }
    const joiningCheck = validateDateOfJoining(dateOfJoining?.trim());
  ...
  const saveEditRow = async (student) => {
    try {
      ...
      const isInternship = !!(
        student.isInternship || isScopedInternshipRole(student.role || savedRole)
      );
      const updatePayload = {
      ...
      const mainStudentDocId = student.studentDocId;
      if (mainStudentDocId) {
        const mainRef = firestoreHelpers.doc(
          db,
    ...
    try {
      ...
      const result = await executeStudentDelete({
        firestore: {
          ...
          writeBatch: firestoreHelpers.writeBatch,
          query: firestoreHelpers.query,
          where: firestoreHelpers.where,
        },
        student,
        studentId,
      ...
      if (result.success) {
        const deletedEmail = student?.email?.trim().toLowerCase() || "";
        setStudents((prev) =>
          prev.filter(
            (s) =>
              s.id !== studentId &&
              s.studentDocId !== studentId &&
              s.uid !== studentId &&
              (!deletedEmail ||
                String(s.email || "").trim().toLowerCase() !== deletedEmail)
          )
        );
        await fetchStudents();
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="date" value={admissionForm.dateOfJoining} onChange={(e) => updateForm("dateOfJoining", e.target.value)} min={todayDateStr} placeholder="dd-mm-yyyy" className="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#00448a]/30 focus:border-[#00448a]" />
                      </div>
                      <div>
        ...
        {!isFirebaseConfigured ? (
          ...
          <>
            ...
            <motion.div
              ...
              className="w-full rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden"
            >
                          ...
                          >
                            ...
                            <td className="p-4 text-slate-600">
                              {isEditing ? (
                                ...
                                formatStudentRoleLabelsForDoc(s)
                              )}
                            </td>
                            <td className="p-4 text-slate-600">
                              {isEditing ? (
