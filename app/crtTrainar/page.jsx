  ...
  for (const progDoc of programsSnap.docs) {
    ...
    for (const batchDoc of batchesSnap.docs) {
      ...
      const trainerIds =
        Array.isArray(batch.trainerIds) && batch.trainerIds.length > 0
          ...
          : batch.trainerId
            ? [batch.trainerId]
            : [];
      if (!trainerIds.includes(trainerUid)) continue;
      ...
      const courseIdAllowList = (() => {
        const entry = Array.isArray(batch.assignedTrainers)
          ? batch.assignedTrainers.find((t) => t.id === trainerUid)
          : null;
        let raw = [];
        ...
        } else if (Array.isArray(entry?.assignedSubjectIds) && entry.assignedSubjectIds.length > 0) {
          raw = entry.assignedSubjectIds;
        } else {
          raw =
        ...
        if (!Array.isArray(raw) || raw.length === 0) {
          const trackType = entry?.trainerTrackType ?? batch.trainerTrackType;
          if (trackType === "technical") return new Set();
          return null;
        }
