...
import { trainerIsAssignedToCourse } from "../../../../../lib/trainerCourseAccess";
...
export default function InternshipCoursePage() {
  ...
  // Load which chapters are unlocked (after chapters + course are loaded)
  useEffect(() => {
    async function loadAccess() {
      if (!user || !courseId || !internshipId) return;

      try {
        const sourceCourseId = course?.sourceCourseId || "";

        const userSnap = await getDoc(doc(db, "users", user.uid));
        const userData = userSnap.exists() ? userSnap.data() : null;

        // Admin-assigned trainers: full access to all chapters (students unchanged)
        if (
          isTrainerUser &&
          ...
          setAccessibleChapters(chapters.map((ch) => ch.id));
          return;
        }
        ...
        if (isTrainerUser && userData) {
          collectChapterAccessIds(
            ...
            courseId,
            sourceCourseId
          ).forEach((id) => rawIds.add(id));

          const unlockSnap = await getDocs(
            query(
              collection(db, "unlocks"),
              where("key", "==", `trainer:${user.uid}`),
              where("courseId", "==", courseId)
            )
          );
          unlockSnap.docs.forEach((d) => {
            const data = d.data() || {};
            if (data.internshipId && data.internshipId !== internshipId) return;
            if (data.chapterId) rawIds.add(data.chapterId);
          });
        } else {
        ...
        setAccessibleChapters(
          normalizeUnlockedChapterIds(chapters, [...rawIds])
        );
      } catch (e) {
    ...
    loadAccess();
  }, [user, courseId, internshipId, isTrainerUser, chapters, course]);
