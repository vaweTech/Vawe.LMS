...
export default function AdmissionForm({ onStudentAdded }) { 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const dobCheck = validateDateOfBirth(formData.dob);
      if (!dobCheck.valid) {
        alert(`❌ ${dobCheck.message}`);
        return;
      }

      const joiningCheck = validateDateOfJoining(formData.dateOfJoining);
      ...
      const payload = {
        ...
        skillSet: formData.skillSet,
        remarks: formData.remarks,
        isInternship: showInternshipAdmission ? formData.isInternship : false,
      ...
      <div className="max-w-5xl mx-auto p-8 bg-white shadow-md rounded-md">
        <div className="flex flex-wrap justify-between items-start gap-4 mb-6">
          ...
          <div className="flex flex-col items-end gap-3 text-right">
            ...
            <p className="text-xs text-slate-500">
              {showInternshipAdmission
                ? "Select Internship, CRT, or both — a student can have both roles."
                : "Select CRT if this is a CRT student admission."}
            </p>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="space-y-8">
